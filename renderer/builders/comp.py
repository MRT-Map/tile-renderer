...
while not e:
    ...
    while not name_confirmed:
        ...
        if name in components.component_ids() or name in new_components.keys():
            print(
                term.red(
                    "component already exists; do you want to override its current value?"
                )
            )
            print(
                term.red(
                    components[name]
                    if name in components.component_ids()
                    else new_components[name]
                )
            )
    ...
    if skin["types"][type_.split(" ")[0]]["type"] == "point":
        ...
        print(
            term.yellow(
                "Nodes for {name}: Choose what to pre-write in node list editor: "
            ),
            end="",
        )
        option = input(
        ...
        print(
            term.yellow(
                "Your text editor will open; input a list of nodes, one node per line. "
            )
            + term.bright_yellow("Remember to save before closing.")
        )
        node_confirmed = False
        while not node_confirmed:
    ...
    print(
        term.yellow(
            "Confirm new component (type 'c' to cancel, anything else to confirm):"
        )
    )
