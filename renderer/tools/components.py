...
def find_ends(
    components: ComponentList, nodes: NodeList
...
# TODO fix types
def to_geo_json(
    component_json: ComponentListJson, node_json: NodeListJson, skin_json: SkinJson
) -> dict:
    """
    ...
    for component_id, component in component_json.items():
        ...
        if component_shape == "point":
            ...
            geo_coords = [
                [list(c) for c in tools.nodes.to_coords(component["nodes"], node_json)]
            ]
            ...
            if "hollows" in component.keys():
                for hollow in component["hollows"]:
                    geo_coords.append(
                        [list(c) for c in tools.nodes.to_coords(hollow, node_json)]
                    )
        ...
        geo_feature["geometry"] = {"type": geo_shape, "coordinates": geo_coords}
