...
def v_tile_coords(
    tiles: list[TileCoord], min_zoom: int, max_zoom: int
) -> Literal[True]:
    """
    ...
    for item in tiles:
        if not isinstance(item, tuple):
            ...
            raise ValueError(
                f"Tile coordinates {item} has {len(item)} values instead of 3"
            )
        for n in item:
            if not isinstance(n, (int, float)):
        ...
        if not min_zoom <= item[0] <= max_zoom:
            raise ValueError(
                f"Zoom value {item[0]} is not in the range {min_zoom} <= z <= {max_zoom}"
            )
...
def v_geo_json(geo_json: dict) -> Literal[True]:
    ...
    main_schema = Schema(
        {
            ...
            "features": [{"type": "Feature", "geometry": dict, "properties": dict}],
        },
        ignore_extra_keys=True,
    )

    point = Schema(
        {"type": "Point", "coordinates": And([int, float], lambda c: len(c) == 2)},
        ignore_extra_keys=True,
    )

    line_string = Schema(
        {"type": "LineString", "coordinates": v_coords}, ignore_extra_keys=True
    )
    ...
    multi_point = Schema(
        {"type": "MultiPoint", "coordinates": v_coords}, ignore_extra_keys=True
    )
    ...
    multi_polygon = Schema(
        {
            "type": "MultiPolygon",
            "coordinates": lambda css: all(
                [all([v_coords(c) and c[0] == c[-1] for c in cs]) for cs in css]
            ),
        },
        ignore_extra_keys=True,
    )
